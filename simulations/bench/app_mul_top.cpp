...
Vapp_mul_top *uut;					// instantiation of module
	...
	for (int i = 0; i < TESTCASES; i++) {
		...
		switch (ops) {
			...
			case 3:		// x < 10000
				...
				break;
			case 4:
				tc[i].a = (std::rand() % (int(std::pow(2,10)- std::pow(2,8)))) + int(std::pow(2,8));
				tc[i].b = (std::rand() % (int(std::pow(2,10)- std::pow(2,8)))) + int(std::pow(2,8));
				break;

			case 5:
				tc[i].a = (std::rand() % (int(std::pow(2,14)- std::pow(2,12)))) + int(std::pow(2,12));
				tc[i].b = (std::rand() % (int(std::pow(2,14)- std::pow(2,12)))) + int(std::pow(2,12));
				break;

			case 6:
				tc[i].a = std::rand() % int(std::pow(2,15));
				tc[i].b = std::rand() % int(std::pow(2,15));
				break;
...
void checkOutput(Vapp_mul_top *uut, Testcase tc) {
...
int main(int argc, char** argv)
{
    ...
    uut = new Vapp_mul_top;   // Create instance
